...
export function EmailForm({ children, subject }: { children: ReactNode; subject: string }) {
  ...
  return <form onSubmit={submit} className="integrated-form text-[#F8F7F4]">
    {children}
    <p role="status" className="mt-4 text-xs leading-5 text-white/80">{opened ? "Se ha solicitado abrir tu aplicación de correo. Revisa el mensaje y envíalo para completar la solicitud. Si no se abre, escríbenos a hola@premiumwork.es." : "Al continuar se abrirá tu aplicación de correo con los datos de tu solicitud para que puedas revisarlos y enviarlos."}</p>
  </form>;
}
