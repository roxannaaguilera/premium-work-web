...
export function Navbar() {
  ...
  return <header className={`fixed inset-x-0 top-0 z-50 border-b transition-all duration-300 ${scrolled ? "border-[#0B1F3A]/10 bg-[#F8F7F4]/94 shadow-sm backdrop-blur" : "border-white/20 bg-[#0B1F3A]/20 text-white backdrop-blur-sm"}`}>
    <nav className="mx-auto flex h-20 max-w-[1600px] items-center justify-between px-5 md:px-8">
      ...
      <div className="hidden items-center gap-7 lg:flex">
        ...
        <div className="relative" onMouseEnter={() => setServicesOpen(true)} onMouseLeave={() => setServicesOpen(false)}>
          ...
          {servicesOpen && <div className={`absolute left-1/2 top-full mt-4 w-60 -translate-x-1/2 border p-2 shadow-xl ${scrolled ? "border-[#0B1F3A]/10 bg-[#F8F7F4] text-[#0B1F3A]" : "border-white/20 bg-[#0B1F3A]/95 text-white backdrop-blur"}`}>{services.map(s => <a key={s} href={`#${serviceId(s)}`} className={`block px-4 py-3 text-sm transition-colors ${scrolled ? "hover:bg-[#0B1F3A] hover:text-white" : "hover:bg-white/10"}`}>{s}</a>)}</div>}
        </div>
    ...
    {menu && <div className={`border-t px-5 pb-5 lg:hidden ${scrolled ? "border-[#0B1F3A]/10 bg-[#F8F7F4] text-[#0B1F3A]" : "border-white/20 bg-[#0B1F3A]/[.97] text-white backdrop-blur"}`}><div className="mx-auto max-w-[1600px]">{links.map(([name, href]) => <a onClick={() => setMenu(false)} className={`block border-b py-3 text-sm font-semibold ${scrolled ? "border-[#0B1F3A]/10" : "border-white/15"}`} href={href} key={name}>{name}</a>)}<p className={`pt-4 text-xs font-bold uppercase tracking-wider ${scrolled ? "text-[#0B1F3A]/55" : "text-white/60"}`}>Servicios</p>{services.map(s => <a onClick={() => setMenu(false)} href={`#${serviceId(s)}`} key={s} className="block py-2 text-sm">{s}</a>)}<a href="#contacto" className="mt-3 block rounded-sm bg-[#C9A227] px-5 py-3 text-center text-sm font-bold text-[#0B1F3A]">Agenda tu cita</a></div></div>}
  </header>;
}
